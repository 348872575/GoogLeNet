...
# 此处要根据自己文件存放路径来改
image_path = os.path.abspath(os.path.join(os.getcwd(), "data"))  # 数据集路径
train_dir = os.path.join(image_path, "train")  # 训练集路径
validation_dir = os.path.join(image_path, "valid")  # 验证集路径
save_path = os.path.join(os.getcwd(),'save_weights')
...
model = GoogLeNet(im_height=im_height, im_width=im_width, class_num=class_num, aux_logits=aux_logits)  # 实例化模型
...
#     save_best_only=True)
#
# trained = model.fit(train_data_gen,
#                     batch_size=batch_size,
#                     epochs=epochs,
